...
from awsome.utils.milvus_client import milvus_client
...
class KnowledgeService(BaseService):
    ...
    def create_knowledge(cls, knowledge_create: KnowledgeCreate):
        ...
        new_milvus_index_name = f"i_awsome_{uuid.uuid4().hex}"
        try:
            # 创建MilvusCollection
            ...
            milvus_client.create_index(new_milvus_collection_name,  # 集合名
                                       "vector",  # 创建索引的属性
                                       new_milvus_index_name,  # unique 索引名
                                       milvus_default_index_params)  # 索引参数
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"创建Milvus集合/索引异常: {str(e)}")
        knowledge_create.collection_name = new_milvus_collection_name
        knowledge_create.index_name = new_milvus_index_name
